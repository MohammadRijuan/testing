...
export default function Sidebar({ isOpen, onClose }) {
  ...
  const getLinkClass = (path) => {
    ...
    const activeClass =
      "bg-indigo-600/10 text-indigo-400 border-l-2 border-indigo-500 rounded-l-none pl-[12px]";
    const inactiveClass =
    ...
    >
      ...
      <nav className="space-y-1.5 flex-1">
        ...
        <Link
          ...
          className={getLinkClass("/dashboard/hero-banners")}
        >
          {/* Premium Wide-Format Media Canvas Icon */}
          <svg
            className={getIconClass("/dashboard/hero-banners")}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth="1.5"
